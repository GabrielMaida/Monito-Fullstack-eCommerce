...
export default function WhatsNew() {
	...
	return (
		<section className="my-15 text-lg">
			...
			</header>
			{
				<div className="relative flex flex-row flex-wrap justify-between gap-y-3 text-[#00171F]">
					{dogs.slice(0, 8).map((dog) => (
						<div
							key={dog.sku}
							className="w-[calc(25%-1rem)] p-3 bg-white rounded-lg text-[1.05rem] shadow-md font-bold flex flex-col gap-2 transition-transform duration-300 hover:scale-105 hover:cursor-pointer"
						>
							<img
								src={dog.images[0]}
								alt={dog.name}
