...
export default function WhatsNew({ howManyProducts }: { howManyProducts: number }) {
	...
	return (
		<section className="my-10 text-lg" id="whatsnew">
			...
			<div className="grid grid-cols-4 gap-6 text-[#00171F]">
				{dogs.slice(0, howManyProducts).map((dog) => (
					<a className="relative" href={endpoint + dog.sku}>
						<div key={dog.sku} className="w-full h-full p-3 bg-white rounded-lg text-[1.05rem] shadow-md font-bold flex flex-col gap-2 transition-transform duration-300 hover:scale-105 hover:cursor-pointer">
